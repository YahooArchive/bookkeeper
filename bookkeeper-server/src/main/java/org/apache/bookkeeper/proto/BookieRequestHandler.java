...
class BookieRequestHandler extends SimpleChannelHandler
    ...
    private void handleTrim(BookieProtocol.Request r, Channel c) {
        assert (r instanceof BookieProtocol.TrimRequest);
        BookieProtocol.TrimRequest trim = (BookieProtocol.TrimRequest)r;

        if (bookie.isReadOnly()) {
            LOG.warn("BookieServer is running as readonly mode,"
                     + " so rejecting the request from the client!");
            c.write(ResponseBuilder.buildErrorResponse(BookieProtocol.EREADONLY, trim));
            if (statsEnabled) {
                bkStats.getOpStats(BKStats.STATS_TRIM).incrementFailedOps();
            }
            return;
        }
        ...
        int rc = BookieProtocol.EOK;
        try {
             bookie.trim(r.ledgerId, r.entryId);
        } catch (NoLedgerException e) {
            LOG.info("Could not trim ledger because it doesn't exist. Ignoring {}@{}", r.entryId, r.ledgerId);
            rc = BookieProtocol.EOK;
        } catch (IOException e) {
            LOG.error("Error writing " + trim, e);
            rc = BookieProtocol.EIO;
        }

        if (rc != BookieProtocol.EOK) {
            c.write(ResponseBuilder.buildErrorResponse(rc, trim));
            if (statsEnabled) {
                bkStats.getOpStats(BKStats.STATS_TRIM).incrementFailedOps();
            }
        } else {
            c.write(ResponseBuilder.buildTrimResponse(trim));
        }
    }
